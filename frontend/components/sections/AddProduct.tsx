...
const AddProduct = ({ state, actions }: AddProductProps) => {
  ...
  <div className="grid grid-cols-2 gap-4">
    {Object.entries(newProduct).map(([key, value]) => {
      const isLargeField = key === "detail";
      const isSmallField = key === "text";
      const isArray = Array.isArray(value);
      return (
        <div key={key} className={isLargeField ? "col-span-2" : ""}>
          <Label htmlFor={key}>{formatLabel(key)}</Label>
          {isLargeField ? (
            <Textarea 
              id={key} 
            ...
            <Input 
              ...
              onChange={(e) => setNewProduct((prev) => ({ ...prev, [key]: e.target.value.split(", ") }))} 
            />
          ) : (
            <Input id={key} name={key} value={value as string} onChange={handleInputChange} />
          )}
        ...
        <DialogContent className="sm:max-w-[800px]">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="grid w-full grid-cols-3">
              <TabsTrigger value="newProduct">New Document</TabsTrigger>
