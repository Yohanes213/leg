...
import { contextMachine, serializeContextState,deserializeContextState } from "./contextMachine";
...
const serializeAppState = (context: any) => {
  return {
    ...
    contextState: serializeContextState(context.context.conRef),
  };
};
...
const deserializeAppState = (savedState: any, spawn: any) => {
  ...
  return {
    ...
    conRef: spawn(contextMachine, { input: deserializeContextState(savedState.contextState) }),
  };
};
...
export const appMachine = setup({
  types: {
    context: {} as {
      ...
      conRef: ActorRefFrom<typeof contextMachine>;
      model: Model;
  ...
  states: {
    open: {
      ...
      on: {
        ...
        "user.selectContextViews": {
          target: "#appActor.open.selectContext",
        },
      },
      states: {
        chatting: {
        ...
        selectContext:{
          entry: sendTo(({ context }) => context.conRef!, {
            type: "app.startManagingContexts",
          }),
          exit: sendTo(({ context }) => context.conRef!, {
