import { Model, ModelSchema, RequestData, RequestDataSchema, ResponseMessage, requestDataToJson, responseMessageFromJson } from "@/types";
...
export const webSocketMachine = setup({
  types: {
    ...
    events: {} as
      | { type: "parentActor.connect"; data: { sessionId: string; model: Model;} }
  ...
  actors: {
    socketConnector: fromPromise(async ({ input }: { input: { sessionId: string; model: Model;} }): Promise<{ socket: WebSocket; data: ResponseMessage }> => {
      return new Promise((resolve, reject) => {
        console.log(SOCKET_URL)
        console.log(input, "gggggggggggggggggggg")
        const socket = new WebSocket(`${SOCKET_URL}${input.sessionId}?model=${input.model}&save_chat=false`)
