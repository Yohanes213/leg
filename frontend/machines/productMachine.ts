...
export const productMachine = setup({
  ...
  actors: {
    productUpdater: fromPromise(async ({ input }: { input: { id: string; productData: Product } }) => {
      ...
      // fewfwffefe
      const response = await apiCall("PUT", `/update/${input.id}`, productToJson(input.productData));
      console.log(response.message)
      console.log(input.productData)
      if (response.message) return input.productData;
      console.log("+++ productUpdater response", response);
      throw new Error("Failed to update product");
    }),
    productDeleter: fromPromise(async ({ input }: { input: { id: string } }) => {
      const response = await apiCall("DELETE", `/update/${input.id}`);
    ...
    productAdder: fromPromise(async ({ input }: { input: { productData: Product } }) => {
      ...
      const response = await apiCall("POST", "/update", productToJson(input.productData));
    ...
    productsFetcher: fromPromise(async ({ input }: { input: { page: number; pageSize: number; filter?: Record<string, string> } }) => {
      ...
      const response = await apiCall("GET", `/getall?${queryParams.toString()}`);
      
      console.log("response", response);
      return {
        products: response["data"]["Get"]["DocumentTemplate"],
        totalProducts: response["data"]["total_templates"],
      };
    }),
  },
  guards: {
                  ...
                  actions: [
                    assign({
                      ...
                      products: ({ context, event }) => {
                        console.log('Products type:', typeof context.products);
                        console.log('Products value:', context);
                        // context.products = context.products["data"]["Get"]["DocumentTemplate"]
                        return context.products.map((p) => (p.id === event.output.id ? event.output : p))
                      },
                    }),
                    emit({
                      type: "notification",
